...
// Generate a perfect level with minimal empty tubes
function generatePerfectLevel(levelIndex) {
...
// Generate perfect tubes for a level
function generatePerfectTubes(level) {
...
// Generate 100 perfect levels
function generate100PerfectLevels() {
  console.log('Generating 100 perfect levels with minimal empty tubes...');
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const levelConfig = generatePerfectLevel(i);
    
    // Generate tubes for this level
    levelConfig.tubes = generatePerfectTubes(levelConfig);
  ...
  if (allPerfect && noConflicts && frozenPartial && emptyTubesMinimal) {
    console.log('\n🎉 SUCCESS! Generated 100 perfect challenging levels:');
