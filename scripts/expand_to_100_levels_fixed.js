...
// Generate challenging level configuration
function generateChallengingLevel(levelIndex) {
  ...
  if (levelIndex < 20) {
    ...
    emptyTubes = 1; // Minimal empty tubes
    frozenTubes = Math.random() > 0.5 ? [Math.floor(Math.random() * 3)] : [];
    oneColorTubes = Math.random() > 0.6 ? [{ tubeIndex: Math.floor(Math.random() * 3) + 3, color: COLORS[Math.floor(Math.random() * colors)] }] : [];
    ...
    emptyTubes = 1; // Minimal empty tubes
    frozenTubes = Math.random() > 0.3 ? [Math.floor(Math.random() * 4), Math.floor(Math.random() * 4) + 4] : [Math.floor(Math.random() * 4)];
    oneColorTubes = Math.random() > 0.4 ? [
      { tubeIndex: Math.floor(Math.random() * 4) + 4, color: COLORS[Math.floor(Math.random() * colors)] },
      { tubeIndex: Math.floor(Math.random() * 4) + 8, color: COLORS[Math.floor(Math.random() * colors)] }
    ] : [{ tubeIndex: Math.floor(Math.random() * 4) + 4, color: COLORS[Math.floor(Math.random() * colors)] }];
    ...
    emptyTubes = 1; // Minimal empty tubes
    frozenTubes = [Math.floor(Math.random() * 5), Math.floor(Math.random() * 5) + 5, Math.floor(Math.random() * 5) + 10];
    oneColorTubes = [
      { tubeIndex: Math.floor(Math.random() * 5) + 5, color: COLORS[Math.floor(Math.random() * colors)] },
      { tubeIndex: Math.floor(Math.random() * 5) + 10, color: COLORS[Math.floor(Math.random() * colors)] }
    ];
  } else {
    ...
    emptyTubes = 1; // Minimal empty tubes
    frozenTubes = [Math.floor(Math.random() * 6), Math.floor(Math.random() * 6) + 6, Math.floor(Math.random() * 6) + 12, Math.floor(Math.random() * 6) + 18];
    oneColorTubes = [
      { tubeIndex: Math.floor(Math.random() * 6) + 6, color: COLORS[Math.floor(Math.random() * colors)] },
      { tubeIndex: Math.floor(Math.random() * 6) + 12, color: COLORS[Math.floor(Math.random() * colors)] },
      { tubeIndex: Math.floor(Math.random() * 6) + 18, color: COLORS[Math.floor(Math.random() * colors)] }
    ];
  }
  
  // Ensure no duplicate frozen tube indices
  frozenTubes = [...new Set(frozenTubes)];
  
  // Ensure one-color tubes don't conflict with frozen tubes
  oneColorTubes = oneColorTubes.filter(restriction => !frozenTubes.includes(restriction.tubeIndex));
  
  // Ensure one-color tubes don't have duplicate indices
  const oneColorIndices = new Set();
  oneColorTubes = oneColorTubes.filter(restriction => {
    if (oneColorIndices.has(restriction.tubeIndex)) return false;
    oneColorIndices.add(restriction.tubeIndex);
    return true;
  });
  ...
  return {
    ...
    levelIndex, // Add level index for reference
    tubes: [] // Will be generated
  };
}
...
function generateTubes(level) {
  ...
  frozenTubes.forEach((frozenIndex) => {
    // Ensure frozen index is within bounds
    if (frozenIndex >= totalTubes) {
      console.log(`Warning: Frozen tube index ${frozenIndex} out of bounds for level ${levelIndex + 1}, skipping`);
      return;
    }
    
    const frozenColor = targetColors[Math.floor(Math.random() * targetColors.length)];
  ...
  oneColorInTubes.forEach(restriction => {
    ...
    // Ensure tube index is within bounds
    if (tubeIndex >= totalTubes) {
      console.log(`Warning: One-color tube index ${tubeIndex} out of bounds for level ${levelIndex + 1}, skipping`);
      return;
    }
...
function expandTo100Levels() {
  ...
  for (let i = startIndex; i < 100; i++) {
    ...
    const levelConfig = generateChallengingLevel(i);
