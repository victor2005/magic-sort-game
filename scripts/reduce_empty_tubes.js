...
// Reduce empty tubes to make game more challenging
function reduceEmptyTubes() {
  console.log('Reducing unnecessary empty tubes to make game more challenging...');
  ...
  levels.forEach((level, levelIndex) => {
    ...
    // Calculate optimal empty tubes based on level complexity
    let optimalEmptyTubes = 1; // Always keep at least 1 empty tube
    
    if (levelIndex < 10) {
      optimalEmptyTubes = 1; // Early levels: 1 empty tube
    } else if (levelIndex < 20) {
      optimalEmptyTubes = 1; // Mid-early levels: 1 empty tube
    } else if (levelIndex < 30) {
      optimalEmptyTubes = 1; // Mid levels: 1 empty tube
    } else if (levelIndex < 40) {
      optimalEmptyTubes = 2; // Mid-late levels: 2 empty tubes max
    } else {
    ...
    if (currentEmptyTubes > optimalEmptyTubes) {
      ...
      frozenTubes.forEach((frozenIndex, frozenIdx) => {
        ...
        if (tube.length > 0) {
          ...
          frozenTubeColors[frozenIndex] = frozenColor;
  ...
  console.log(`\n🎉 Reduced ${totalReduced} unnecessary empty tubes!`);
  ...
  if (allPerfect && noConflicts && frozenPartial) {
    console.log('\n🎉 SUCCESS! Game is now more challenging:');
    ...
    console.log('   • Reduced unnecessary empty tubes');
    console.log('   • Game is now more challenging and fun!');
  } else {
