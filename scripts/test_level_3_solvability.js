...
// Get Level 3 (index 2)
const level3 = levelsData[2];
console.log('🎯 Testing Level 3 Solvability...\n');

console.log('📊 Level 3 Configuration:');
console.log(`Colors: ${level3.colors}`);
console.log(`Tube Size: ${level3.tubeSize}`);
console.log(`Empty Tubes: ${level3.emptyTubes}`);
console.log(`Frozen Tubes: ${level3.frozenTubes.length}`);
console.log(`One-Color Tubes: ${level3.oneColorInTubes.length}`);

console.log('\n🎨 Tubes:');
level3.tubes.forEach((tube, index) => {
...
Object.entries(colorCounts).forEach(([color, count]) => {
  ...
  if (count !== level3.tubeSize) {
    console.log(`    ⚠️  WARNING: ${color} has ${count} segments, should be ${level3.tubeSize}`);
  }
});
...
console.log('\n✅ Is Level 3 already solved?', isSolved(level3.tubes, level3.tubeSize));

// Check if any tubes are already completed
console.log('\n🎯 Completed Tubes:');
level3.tubes.forEach((tube, index) => {
  if (tube.length === level3.tubeSize && tube.every(color => color === tube[0])) {
...
console.log('\n🔍 Has possible moves?', hasPossibleMove(level3.tubes, level3.tubeSize));
...
console.log('\n🧪 Attempting to solve Level 3...');
const startTime = Date.now();
const solvable = solveLevel(level3.tubes, level3.tubeSize);
const endTime = Date.now();

console.log(`⏱️  Solver took ${endTime - startTime}ms`);
if (solvable) {
  console.log('✅ Level 3 is SOLVABLE!');
} else {
  console.log('❌ Level 3 appears to be UNSOLVABLE or very difficult');
