...
console.log('🎯 Regenerating All Levels with Proper Difficulty Progression...\n');
...
function getExpectedMinMoves(levelIndex) {
  if (levelIndex < 3) return 4; // Tutorial levels: 4 moves
  if (levelIndex < 20) return 8; // Early levels: 8 moves
  if (levelIndex < 40) return 12; // Mid levels: 12 moves
  if (levelIndex < 60) return 18; // Advanced levels: 18 moves
  if (levelIndex < 80) return 25; // Expert levels: 25 moves
  return 35; // Master levels: 35 moves
}
...
// Create a properly configured level
function createProperLevel(levelIndex) {
  ...
  // Apply frozen tube restrictions
  config.frozenTubes.forEach((frozenIndex, frozenIdx) => {
    ...
    // Clear the frozen tube
    solvedTubes[frozenIndex] = [];
    
    // Add partial frozen segments
    for (let i = 0; i < frozenSegments; i++) {
      solvedTubes[frozenIndex].push(frozenColor);
    }
    
    // Remove the frozen color from other tubes and redistribute
    const remainingSegments = config.tubeSize - frozenSegments;
    let segmentsToRedistribute = 0;
    
    // Count how many segments of this color are in other tubes
    for (let i = 0; i < solvedTubes.length; i++) {
      if (!config.frozenTubes.includes(i)) {
        const colorCount = solvedTubes[i].filter(c => c === frozenColor).length;
        segmentsToRedistribute += colorCount;
  ...
  // Apply one-color tube restrictions
  config.oneColorInTubes.forEach((oneColorConfig, oneColorIdx) => {
    ...
    // Check if this color conflicts with frozen tubes
    const frozenColors = config.frozenTubes.map((_, idx) => levelColors[idx]);
    if (frozenColors.includes(oneColorColor)) {
      // Choose a different color that's not used in frozen tubes
    ...
    if (levelIndex >= 40) {
      // Create mixed colors for one-color tubes
      const mixedColors = [];
    ...
    // Redistribute the remaining segments of this color
    const totalSegments = config.tubeSize;
    const usedSegments = solvedTubes[oneColorIndex].filter(c => c === oneColorConfig.color).length;
    const remainingSegments = totalSegments - usedSegments;
    
    // Remove this color from other tubes and redistribute
    let segmentsToRedistribute = 0;
    for (let i = 0; i < solvedTubes.length; i++) {
      if (i !== oneColorIndex && !config.frozenTubes.includes(i)) {
        const colorCount = solvedTubes[i].filter(c => c === oneColorConfig.color).length;
        segmentsToRedistribute += colorCount;
  ...
  // Now scramble the level with proper difficulty
  let bestLevel = null;
  let bestMoves = 0;
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    // Apply aggressive scrambling based on difficulty
    const baseMoves = Math.max(config.minMoves * 2, 15); // At least 15 moves
    const numMoves = Math.floor(Math.random() * 30) + baseMoves;
    const moves = [];
    ...
    // Test if solvable and meets difficulty requirements
    const result = solveLevel(scrambledTubes, config.tubeSize);
    
    if (result.solvable && result.moves >= config.minMoves) {
...
// Generate all levels
console.log('🎯 Generating all 100 levels with proper difficulty progression...\n');
...
for (let i = 0; i < 100; i++) {
  ...
  const { level: newLevel, moves: newMoves } = createProperLevel(i);
  
  if (newLevel && newMoves > 0) {
    console.log(`✅ Level ${i + 1} generated! (${newMoves} moves)`);
    allLevels.push(newLevel);
  } else {
    console.log(`❌ Failed to generate Level ${i + 1}`);
...
console.log('\n🎉 All 100 levels have been regenerated with proper difficulty progression!');
