...
// Fix conflicts between frozen tubes and one-color tubes
function fixFrozenOneColorConflicts(level, levelIndex) {
  ...
  console.log(`\nFixing Level ${levelIndex + 1} frozen/one-color conflicts:`);
  ...
  oneColorInTubes.forEach(restriction => {
    ...
    if (conflictingFrozenTube !== undefined) {
      ...
      // No conflict, use the designated color (partial amount)
      const partialAmount = Math.floor(Math.random() * (level.tubeSize - 1)) + 1; // 1 to tubeSize-1
...
// Main execution
console.log('Fixing conflicts between frozen tubes and one-color tubes...');

// Load levels
const levelsPath = 'src/levels.json';
const levels = JSON.parse(fs.readFileSync(levelsPath, 'utf8'));

// Find levels with both frozen tubes and one-color tubes
let levelsWithConflicts = [];
levels.forEach((level, levelIndex) => {
  const frozenTubes = level.frozenTubes || [];
  const oneColorInTubes = level.oneColorInTubes || [];
  if (frozenTubes.length > 0 && oneColorInTubes.length > 0) {
    levelsWithConflicts.push(levelIndex);
  }
});

console.log(`Found ${levelsWithConflicts.length} levels with both frozen and one-color tubes`);

// Fix each level with potential conflicts
levelsWithConflicts.forEach(levelIndex => {
  fixFrozenOneColorConflicts(levels[levelIndex], levelIndex);
});

// Save the fixed levels
fs.writeFileSync(levelsPath, JSON.stringify(levels, null, 2));
console.log(`\nFixed ${levelsWithConflicts.length} levels with frozen/one-color conflicts`);
